...
ui <- fluidPage(
  titlePanel("Show And Download Geodata"),
  
  sidebarLayout(
    sidebarPanel(
      selectInput("mainfolder", "Select Mainfolder:", 
                  choices = c(None='.', main_folders), width = "100%"),
      uiOutput("subfolderUI"),
    
      shinyDirButton("directory", "Choose Directory and Download", "Please select a directory")
    ),
    
    mainPanel(
      uiOutput("iframeOutput")      # Dynamic iframe
    )
  )
)
...
server <- function(input, output, session) {
  ...
  observeEvent(input$mainfolder, {
    ...
    updateSelectInput(session, "subfolder", 
  ...
  output$subfolderUI <- renderUI({
    selectInput("subfolder", "Select Subfolder:", 
                choices = NULL) # Initially empty; will be updated based on mainfolder
  })
  
  shinyDirChoose(input, "directory", roots = c(home = getwd()), session = session)
  
  selected_dir <- reactive({
    req(input$directory)
    parseDirPath(c(home = getwd()), input$directory)
  ...
  observeEvent(input$directory, {
    ...
    withProgress(message = 'Copying files...', value = 0, {
      ...
      if (num_files == 0) {
        showNotification("No files to copy!", type = "warning")
        return(NULL)
      }
      ...
      showNotification(paste("Folder copied to:", selected_dir()), type = "message")
    })  
  })

  # Render the iframe dynamically based on the selected mainfolder and subfolder
  output$iframeOutput <- renderUI({
