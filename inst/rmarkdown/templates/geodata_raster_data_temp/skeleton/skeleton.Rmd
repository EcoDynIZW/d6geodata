...
meta$sub_name <- paste0(str_c(stringr::str_split(meta$name, "_")[[1]] %>% str_sub(1,2), collapse = ""))
...
meta$sub_name <- paste0(str_c(stringr::str_split(meta$name, "_")[[1]] %>% str_sub(1,2), collapse = ""), id)

DBI::dbGetQuery(con, glue::glue("UPDATE envdata.metadata SET sub_name='",meta$sub_name,"' WHERE geo_id=",id$geo_id,";"))
...
if(nlyr(tif) >= 10){
      ...
      DBI::dbGetQuery(con, glue::glue("UPDATE envdata.metadata 
...
# cheack if number of cells is to large
   ...
   DBI::dbExecute(con, glue::glue("ALTER TABLE envdata.",
                              meta$sub_name,
                              " ADD geo_id integer NULL"))

    DBI::dbExecute(con, glue::glue("UPDATE envdata.",
                              meta$sub_name,
                              " SET geo_id = ",
                              id$geo_id))
