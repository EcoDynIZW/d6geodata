...
sf::st_write(data, here(path, stringi::stri_replace_last_fixed(data_name, "_", ".")), overwrite = TRUE)
```
...
```{r}
data_sf <- data %>% dplyr::select()

column <- ''

p_base_map <-
  ggplot() + geom_sf(data = data_sf, aes(fill = column, group = group), color = NA) 
 
if(meta$type_of_data %in% c('continuous_numeric', 'discrete_numeric')) {
  p_map <- p_base_map + rcartocolor::scale_fill_carto_d(palette = "Emrld", 
...
if(meta$type_of_data %in% c('unordered_categorical', 'ordered_categorical')) {
 p_base_map +
    rcartocolor::scale_fill_carto_c(
      palette = "Bold",
      breaks = as.numeric((table((data %>% data.frame())[, column]) %>%
                  data.frame() %>%
                  dplyr::mutate(Var1 = 1:n()) %>% 
                  dplyr::filter(Var1 %in% base::seq(1, max(Var1), round(max(Var1)/5))))$'Var1'),
      name = NULL
    ) +
