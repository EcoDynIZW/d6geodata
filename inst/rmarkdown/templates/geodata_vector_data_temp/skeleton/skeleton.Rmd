...
```{r}
col_nam <- base::readline("Enter column name:")

data_sf <- data %>% dplyr::select(col_nam)

p_base_map <-
  ggplot() + geom_sf(data = data_sf, aes(fill = col_nam), color = NA) 
...
if(meta$type_of_data %in% c('unordered_categorical', 'ordered_categorical')) {
  ...
  data_sf <- data_sf |>
    group_by(col_nam) |>
    mutate(label = ifelse(col_nam%in% droplevels(breaks$Var1)[-1],
                          col_nam,
                          "other"))
